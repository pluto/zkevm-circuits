...
impl<F: Field> ExecutionConfig<F> {
    ...
    pub(crate) fn configure(
        ...
        // Initialize 128 advice columns (approximately 32 bytes each)
        // Of those, 28 are lookups. 
    ...
    fn check_rw_lookup(
        ...
        if step.execution_state() == ExecutionState::BeginTx {
            ...
            for i in 0..step.rw_indices_len() {
                ...
                println!(
                    ...
                    i,
                    rw,
                    rw.table_assignment().unwrap().rlc(lookup_randomness)
                );
            }
        }
        ...
        for (name, value) in assigned_rw_values.iter() {
            ...
            if !rlc_assignments.contains(value) {
                log::error!(
                    "rw lookup error: name: {}, value:{:?}, step: {:?}",
                    *name,
                    value,
                    step
                );
