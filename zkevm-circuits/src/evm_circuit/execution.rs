...
impl<F: Field> ExecutionConfig<F> {
    ...
    fn check_rw_lookup(
        ...
        // For BeginTx, len=31 with removed AccountWrite Nonce 
        //              len=32 with added AccountWrite Nonce 
        if step.execution_state() == ExecutionState::BeginTx {
            println!("ASSIGNED LOOKUPS: len={}, {:?}", assigned_rw_values.len(), assigned_rw_values);
            for i in 0..step.rw_indices_len() {
                let rw = block.get_rws(step, i);
                println!("Actual RWS {}: {:?}, RLC:{:?}", i, rw, rw.table_assignment().unwrap().rlc(lookup_randomness));
            }
        }
        ...
        for (name, value) in assigned_rw_values.iter() {
            ...
            if !rlc_assignments.contains(value) {
                log::error!("rw lookup error: name: {}, value:{:?}, step: {:?}", *name, value, step);
            }
        }
