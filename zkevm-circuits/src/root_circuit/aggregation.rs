...
/// for `Halo2Loader`.
pub type BaseFieldEccChip<'chip, C> = halo2_ecc::ecc::BaseFieldEccChip<'chip, C>;
/// `Halo2Loader` with hardcoded `EccChip`.
pub type Halo2Loader<'chip, C> = loader::halo2::Halo2Loader<C, BaseFieldEccChip<'chip, C>>;
// pub type EccChip<C> = halo2_wrong_ecc::BaseFieldEccChip<C, LIMBS, BITS>;
//pub type Halo2Loader<C> = loader::halo2::Halo2Loader<C, EccChip<C>>;
...
impl<'a, C: CurveAffine> SnarkWitness<'a, C> {
    ...
    // fn loaded_instances(
    //     &self,
    //     loader: &Rc<Halo2Loader>,
    // ) -> Vec<Vec<Scalar<C, BaseFieldEccChip>>> {
    //     self.instances
...
impl AggregationConfig {
    ...
    //             M::G1Affine,
    //             Rc<Halo2Loader<M::G1Affine>>,
    //             VerifyingKey = KzgSvk<M>,
    //             Output = KzgAccumulator<M::G1Affine, Rc<Halo2Loader<M::G1Affine>>>,
    //         > + AccumulationScheme<
    //             M::G1Affine,
    //             Rc<Halo2Loader<M::G1Affine>>,
    //             Accumulator = KzgAccumulator<M::G1Affine, Rc<Halo2Loader<M::G1Affine>>>,
    //             VerifyingKey = KzgAsVerifyingKey,
    //         >,
    // {
    //     type PoseidonTranscript<'a, C, S> =
    //         transcript::halo2::PoseidonTranscript<C, Rc<Halo2Loader<C>>, S, T, RATE, R_F, R_P>;
...
where
    ...
    M::G2Affine: SerdeObject + CurveAffine<ScalarExt = M::Fr, CurveExt = M::G2>,
    M::Fr: Field,
    for<'b> As: PolynomialCommitmentScheme<
            M::G1Affine,
            NativeLoader,
        ...
        > + AccumulationDecider<M::G1Affine, NativeLoader, DecidingKey = KzgDecidingKey<M>>,
{
    ...
    let accumulators = snarks
        .into_iter()
        .map(|snark| {
            let mut transcript = PoseidonTranscript::new::<SECURE_MDS>(snark.proof);
            let proof = PlonkSuccinctVerifier::<As>::read_proof(
                &svk,
                snark.protocol,
                snark.instances,
            ...
            let sp = snark.protocol.preprocessed.iter().len();
            let pw = proof.witnesses.iter().len();
            let pq = proof.quotients.iter().len();
            println!(" === DEBUG (ROOT CIRCUIT): sp_len={}, pw_len={}, pq_len={}", sp, pw, pq);
            let mut count = 0;
            ...
            println!(" === DEBUG (ROOT CIRCUIT): count_identity={}, valid_prior_to_invalid={}", count, valid_count);
            if count > 0 {
            ...
            let r = PlonkSuccinctVerifier::verify(&svk, snark.protocol, snark.instances, &proof);
            println!(" === DEBUG (ROOT CIRCUIT): Completed verification");
            r
        })
    ...
    println!(" === DEBUG (ROOT CIRCUIT): Accumulator created, attempting deciding.");
    As::decide(&dk, accumulator.clone())?;
    println!(" === DEBUG (ROOT CIRCUIT): Decider successful.");
